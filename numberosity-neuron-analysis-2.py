...
def analyze_tuning_width(tuning_curves, selective_neurons, preferred_numerosities, 
    ...
    for i, (scale_name, scale_func) in enumerate(scales.items()):
        ...
        if len(prefs) >= 2:
            try:
                ...
                print(f"Could not calculate correlation for {scale_name} scale: {str(e)}")
                ax.text(0.5, 0.9, "Insufficient data for correlation", 
                       transform=ax.transAxes, ha='center')
        else:
            ax.text(0.5, 0.5, f"Not enough data points ({len(prefs)})", 
                   transform=ax.transAxes, ha='center')
    ...
    # Add slope values only if we have enough data points
    for scale_name in ['linear', 'log2']:
        if scale_name in scale_sigmas and len(scale_sigmas[scale_name]) >= 2 and len(prefs) >= 2:
            try:
                results[f'slope_{scale_name}'] = float(np.polyfit(prefs, scale_sigmas[scale_name], 1)[0])
            except Exception as e:
                print(f"Could not calculate slope for {scale_name} scale: {str(e)}")
                results[f'slope_{scale_name}'] = 0.0
        else:
            results[f'slope_{scale_name}'] = 0.0
